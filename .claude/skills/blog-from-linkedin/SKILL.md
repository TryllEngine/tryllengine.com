...
**This is the FIRST thing to do after determining metadata.** Every blog post needs a cover image — a geometric editorial illustration, NEVER a photo. Photos of people, screenshots, etc. are separate "inline" images that go in the markdown body (Step 4).

The cover image is used in two places:
- **Blog post page**: shown as a 168x168px hero image in the HTML header
- **Blog listing page**: shown as a 4:3 card thumbnail

**Prompt crafting is delegated to the `blog-cover-prompts` skill.** Invoke it with the post text — it produces 3–4 diverse, concrete, recognizable variants (chip, iceberg, magnifying glass, doorway with light, etc.) using a locked visual baseline. That skill is the source of truth for style, palette, and iconography per post type.

**Recommended dimensions:** 1024×768 (4:3, matches the blog listing card aspect and avoids top/bottom crop).

**Generating the image:**

1. Use `blog-cover-prompts` to produce prompt variants and present them to the user for selection.
2. If a `.env` with DALL-E API key exists, run the generation script:

```bash
python scripts/generate_hero_image.py --prompt "CHOSEN_PROMPT" --output "assets/blog/{slug}-cover.png"
```

3. If the script fails (no API key, API error) or if the user prefers to generate manually, provide the chosen prompt and wait for them to save the file. Save as `assets/blog/{slug}-cover.png` (PNG) or `.jpg` (recompressed).

**Post-processing:** if the source PNG is >400 KB, convert to JPG at quality 92 for size savings (illustration-style covers compress 4–6× smaller as JPG with no visible loss) and update the `.png` → `.jpg` reference in `posts.json` and the post's HTML page (`image` field + `og:image` + `twitter:image`).
